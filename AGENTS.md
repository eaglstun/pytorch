...
# AI Engineering Policy - MANDATORY

Read `AI_POLICY.md`. This fork uses an AI-first engineering model: nearly all
fork-specific improvements are designed, implemented, tested, and documented by
AI agents working under the direction of the human maintainer. AI execution is
expected and accepted for development in this repository.

The upstream PyTorch contribution restrictions recorded in `AI_POLICY.md` apply
only when interacting with upstream PyTorch. Do not apply upstream's ban on
fully agent-generated contributions to work developed for this fork. Always
represent AI involvement accurately and remain within the scope authorized by
the user.
