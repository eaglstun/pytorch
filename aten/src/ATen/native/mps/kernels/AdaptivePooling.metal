...
kernel void adaptive_avg_pool2d_forward(
  ...
  const ulong input_y_start = output_y * ulong(params.input_height) / output_height;
  const ulong input_y_end = ((output_y + 1) * ulong(params.input_height) + output_height - 1) / output_height;
  const ulong input_x_start = output_x * ulong(params.input_width) / output_width;
  const ulong input_x_end = ((output_x + 1) * ulong(params.input_width) + output_width - 1) / output_width;

  const ulong input_base = batch * ulong(params.input_strides[0]) + channel * ulong(params.input_strides[1]);
  float sum = 0.0f;
  for (ulong input_y = input_y_start; input_y < input_y_end; ++input_y) {
    for (ulong input_x = input_x_start; input_x < input_x_end; ++input_x) {
      sum += float(input[input_base + input_y * ulong(params.input_strides[2]) +
                              input_x * ulong(params.input_strides[3])]);
    }
  }
  const float count = float((input_y_end - input_y_start) * (input_x_end - input_x_start));
  const ulong output_storage_index = batch * ulong(params.output_strides[0]) +
      channel * ulong(params.output_strides[1]) + output_y * ulong(params.output_strides[2]) +
...
kernel void adaptive_avg_pool2d_backward(
  ...
  const ulong output_y_start = input_y * ulong(params.output_height) / input_height;
  const ulong output_y_end = ((input_y + 1) * ulong(params.output_height) + input_height - 1) / input_height;
  const ulong output_x_start = input_x * ulong(params.output_width) / input_width;
  const ulong output_x_end = ((input_x + 1) * ulong(params.output_width) + input_width - 1) / input_width;

  const ulong output_base = batch * ulong(params.output_strides[0]) + channel * ulong(params.output_strides[1]);
  float sum = 0.0f;
  for (ulong output_y = output_y_start; output_y < output_y_end; ++output_y) {
    const ulong input_y_start = output_y * input_height / ulong(params.output_height);
    const ulong input_y_end =
        ((output_y + 1) * input_height + ulong(params.output_height) - 1) / ulong(params.output_height);
    for (ulong output_x = output_x_start; output_x < output_x_end; ++output_x) {
      const ulong input_x_start = output_x * input_width / ulong(params.output_width);
      const ulong input_x_end =
          ((output_x + 1) * input_width + ulong(params.output_width) - 1) / ulong(params.output_width);
      const float count = float((input_y_end - input_y_start) * (input_x_end - input_x_start));
      sum += float(grad_output[output_base + output_y * ulong(params.output_strides[2]) +
                                   output_x * ulong(params.output_strides[3])]) /
          count;
    }
  }
  const ulong input_storage_index = batch * ulong(params.input_strides[0]) + channel * ulong(params.input_strides[1]) +
      input_y * ulong(params.input_strides[2]) + input_x * ulong(params.input_strides[3]);
